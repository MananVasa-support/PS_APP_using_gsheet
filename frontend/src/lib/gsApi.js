 ...
 *
 * Every request carries the signed-in user's Supabase id (userId), so the
 * server stores each user's data in their own folder. The web app surface:
 ...
 *
 * Timing: every call logs its ms; window.apiTimings() console.tables them.
 */
...
// ── App session (sessionStorage; set by authService after Supabase login) ────
...
async function call(route, payload = {}) {
  ...
  for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
    ...
    try {
      ...
      if (err instanceof ApiError && err.code && !RETRYABLE.test(err.message)) {
