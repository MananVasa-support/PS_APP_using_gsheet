...
export default function WaitingApproval() {
  ...
  async function refresh() {
    ...
    try {
      ...
      setUser((u) => ({ ...u, ...fresh }));
      if (fresh?.status === 'Approved') navigate('/home', { replace: true });
    } finally {
      setChecking(false);
    }
  }
  ...
  return (
    <div className="grid min-h-screen place-items-center bg-auth-radial px-4 text-center">
      <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }} className="w-full max-w-md">
        ...
        <div className="card p-8">
          ...
          <h1 className="mt-5 font-display text-2xl font-bold text-white">Waiting for approval</h1>
          ...
          {user?.clientId && (
            <div className="mx-auto mt-5 inline-flex items-center gap-2 rounded-xl bg-ink-800 px-3 py-2 font-mono text-sm text-slate-200">
