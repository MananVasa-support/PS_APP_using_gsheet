...
| **Google Calendar (Power Planner export + Meeting schedule)** | sign in once → export plan/meetings to the user's calendar; re-export updates, never duplicates (map in DB) | **(1)** Google Cloud Console → OAuth Client ID (Web) → origins `http://localhost:3000` (+ live URL later). **(2)** `frontend/.env`: `VITE_GOOGLE_CLIENT_ID=…`. **(3)** Silent-token Edge Function = next step (needs the client secret). | ⚠️ **create the OAuth Client ID** (steps in chat) |
