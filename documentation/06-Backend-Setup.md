...
| **Power Planner data (WIRED)** | weekly PLAN rows + auto-computed REVIEW scoreboard + settings (start date, schedule, custom names, Calendar event map) | **Run schema.sql once more** (adds `power_planner_reviews` + `gcal_event_ids` col). View: **`power_planner_weeks`** (one row per user-week, `data` jsonb = full plan), **`power_planner_reviews`** (completion %, score — real columns), **`power_planner_settings`** (one row per user). | ⚠️ **re-run schema.sql** |
| **Google Calendar (Power Planner export + Meeting schedule)** | **sign in ONCE → silent exports forever** (refresh token server-side); re-export/re-schedule UPDATES events, never duplicates | DONE: OAuth client (ID in `frontend/.env`), redirect URI `…/functions/v1/gcal/callback`, Edge Function **`gcal`** deployed (Verify JWT OFF, secrets `GOOGLE_CLIENT_ID`/`GOOGLE_CLIENT_SECRET` in the vault), tables `google_tokens` + `gcal_oauth_states`. ⚠️ **Re-paste `backend/supabase/functions/gcal/index.ts` + Deploy once more** (callback now lands users on calendar.google.com; account chooser always). **Launch:** publish the consent screen + Google verification (removes the "do you trust …supabase.co" testing warning); add the live domain to origins. | ⚠️ one re-deploy pending |
| **Reasons Eliminator data (WIRED — LAST TOOL, all 5 done)** | assessments (reasons + TFCR categories/subcategories + Power Words + archive), per-reason latest grip score, grip-test run history; PP weekly-review reasons sync in marked `source='power-planner'` | **Re-run schema.sql once** (replaces the 3 empty placeholder tables with real shapes via a safe one-time migration). View: **`reasons_sessions`** (one row per assessment; `reasons` jsonb), **`reasons_grip_tests`** (one row per reason's latest score — `score` is a real column), **`reasons_grip_history`** (one row per completed grip run). | ⚠️ **re-run schema.sql** |
